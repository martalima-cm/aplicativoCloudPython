from command import SerialComm
...
class PiArm(SerialComm):
    ...
    def _checksum(self, data, waitForResponse = True):
        ...
        response = self.write(data, waitForResponse = waitForResponse)
        return response
    ...
    def adjustAngleOffset_write(self, ID=1, offset=0):
        '''
        Adjust angle offset to zero
        '''
        NOB = 4
        if offset < 0:
            offset = 256 + offset
        self._checksum([self.STARTBYTE, self.STARTBYTE, ID, NOB, self.SERVO_ANGLE_OFFSET_WRITE, offset])
    ...
    def torqueServo_Read(self, ID):
        '''
        Enable/Disable Servo Torque 
        '''
        NOB = 3
        self._checksum([self.STARTBYTE, self.STARTBYTE, ID, NOB, self.SERVO_LOAD_OR_UNLOAD_READ], waitForResponse = True)
    ...
    def servoWrite(self, ID=1, position=0, time=1500):
        '''
